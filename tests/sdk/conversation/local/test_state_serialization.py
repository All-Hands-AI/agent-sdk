...
def test_local_conversation_switch_llm_persists_profile(tmp_path, monkeypatch):
    ...
    registry = LLMRegistry()
    base_llm = LLM(model="gpt-4o-mini", service_id="test-llm")
    registry.save_profile("base", base_llm)
    alt_llm = LLM(model="gpt-4o", service_id="alternate", temperature=0.4)
    ...
    assert conversation.agent.llm.service_id == "test-llm"
...
def test_local_conversation_switch_llm_inline_mode_rejected(tmp_path, monkeypatch):
    ...
    base_llm = LLM(model="gpt-4o-mini", service_id="test-llm")
    registry.save_profile("base", base_llm)
    registry.save_profile("alt", LLM(model="gpt-4o", service_id="alternate"))
